...
import 'package:srcat/libs/sr/services/tools/warp/utils.dart';
...
class _WarpRecordPanelState extends State<WarpRecordPanel> {
  ...
  void initState() {
    ...
    timesRange = SrWrapToolServiceUtils.timeRange(times);

    star5 = SrWrapToolServiceUtils.parseGuaranteed(widget.data, type: widget.type);
    resultData = SrWrapToolServiceUtils.parseStar5(widget.data);
  }

  Widget _header() {
    Widget icon = const SCIcon(FluentIcons.starburst, size: 16, weight: FontWeight.w600);
  ...
  Widget _content() {
    ...
    if (!timesRange[0].isNaN && !timesRange[1].isNaN) {
      time = "${SCUtils.unixTimeToStr(timesRange[0], format: timeFormat)}-${SCUtils.unixTimeToStr(timesRange[1], format: timeFormat)}";
    } else if (!timesRange[0].isNaN && timesRange[1].isNaN) {
      time = "${SCUtils.unixTimeToStr(timesRange[0], format: timeFormat)}";
    }
